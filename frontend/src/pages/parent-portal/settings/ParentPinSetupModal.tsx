/**
 * Parent Portal PIN Setup Modal - Set, Change, or Remove PIN
 */
...
import { useRemoveParentPin } from "@api/mutations/useRemoveParentPin";

interface ParentPinSetupModalProps {
  isOpen: boolean;
  onClose: () => void;
  hasExistingPin: boolean;
}

export default function ParentPinSetupModal({ isOpen, onClose, hasExistingPin }: ParentPinSetupModalProps) {
  const { t } = useTranslation(["auth"]);
  const [mode, setMode] = useState<"set" | "remove">("set");
  const [pin, setPin] = useState("");
  const [confirmPin, setConfirmPin] = useState("");
  const setPinMutation = useSetParentPin();
  const removePinMutation = useRemoveParentPin();
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            ...
            {hasExistingPin
              ? mode === "remove"
                ? t("auth:parent_pin.remove_pin")
                : t("auth:parent_pin.change_pin")
              : t("auth:parent_pin.set_pin")}
          </h2>
          <button
            onClick={handleClose}
        ...
        {hasExistingPin && mode === "set" && (
          <div className="mb-4">
            <button
              onClick={() => setMode("remove")}
              className="text-sm text-red-600 hover:text-red-700 underline"
            >
              {t("auth:parent_pin.remove_instead")}
            </button>
          </div>
        )}

        {hasExistingPin && mode === "remove" && (
          <div className="mb-4">
            <button
              onClick={() => setMode("set")}
              className="text-sm text-blue-600 hover:text-blue-700 underline"
            >
              {t("auth:parent_pin.change_instead")}
            </button>
          </div>
        )}
        ...
        {mode === "remove" ? (
          <div className="space-y-4">
            <p className="text-gray-600">{t("auth:parent_pin.remove_confirmation")}</p>
            <div className="flex gap-3">
              ...
              >
                {removePinMutation.isPending ? t("auth:parent_pin.removing") : t("auth:parent_pin.remove")}
              </button>
            </div>
          </div>
        ) : (
