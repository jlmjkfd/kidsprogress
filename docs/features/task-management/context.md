...
**✅ Fixed - Virtual Task Generation**:
- Timezone mismatch in RRULE expansion causing virtual instances to fail for FREQ=DAILY
- Added `timezone.utc` to datetime comparisons in `virtual_instance_service.py`
- Virtual instances now generate correctly for all recurrence patterns

**✅ Fixed - Calendar Consistency**:
- Templates were showing in calendar display (should only show virtual instances)
- Unified calendar filtering between parent and child portals
- Both portals now use same `TaskCalendar` component with consistent behavior

**⚠️ Known Limitation - Virtual Task Materialization**:
- Virtual tasks (ID format: `{template_id}_{date}`) cannot be started/completed yet
- Need to implement materialization: convert virtual task to real DB record on interaction
- Need to add exception to template to prevent duplicate virtual instance generation
- After materialization, task persists even after falling outside 90-day window

**Technical Details**:
- Virtual instances generated on-demand in sliding 90-day window (today-30 to today+60)
- Templates have `is_recurring=true, is_virtual=false`
- Virtual instances have `is_recurring=false, is_virtual=true`
- One-time tasks have `is_recurring=false, is_virtual=false`
...
- **Virtual task lifecycle**: Need materialization logic for start/complete operations
