...
function ScoreBar({ score, label }: { score: number; label: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="w-8 text-sm font-medium text-gray-700">{score}/10</span>
    </div>
  );
}
...
export default function WritingCompletionViewer({
  ...
  const { detailed_data, measured_data, llm_analysis, completed_at } =
    completion;
      ...
      <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
        <div className="border-b bg-gradient-to-r from-blue-50 to-purple-50 px-6 py-4">
          ...
          <div className="mt-2 flex items-center gap-4 text-sm text-gray-600">
            <span>
              {t("tasks:completion.word_count", {
                count: measured_data.word_count,
              })}
            </span>
            <span>•</span>
        ...
        {detailed_data.prompts_used &&
          detailed_data.prompts_used.length > 0 && (
            <div className="border-b bg-blue-50 px-6 py-3 text-sm">
              <span className="font-medium text-blue-800">
                {t("tasks:completion.prompt_used")}:{" "}
              </span>
              <span className="text-blue-700">
                {detailed_data.prompts_used.join("; ")}
              </span>
            </div>
          )}
        ...
        <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
          ...
          <div className="space-y-6 p-6">
            ...
            {llm_analysis.overall_score && (
              <div className="flex items-center gap-4 rounded-lg bg-gradient-to-r from-yellow-50 to-orange-50 p-4">
            ...
            {/* Detailed Scores */}
            {llm_analysis.scores &&
              Object.keys(llm_analysis.scores).length > 0 && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {llm_analysis.scores.grammar !== undefined && (
                      <ScoreBar
                        score={llm_analysis.scores.grammar}
                        label={t("tasks:completion.score_grammar")}
                      />
                    )}
                    {llm_analysis.scores.vocabulary !== undefined && (
                      <ScoreBar
                        score={llm_analysis.scores.vocabulary}
                        label={t("tasks:completion.score_vocabulary")}
                      />
                    )}
                    {llm_analysis.scores.creativity !== undefined && (
                      <ScoreBar
                        score={llm_analysis.scores.creativity}
                        label={t("tasks:completion.score_creativity")}
                      />
                    )}
                    {llm_analysis.scores.structure !== undefined && (
                      <ScoreBar
                        score={llm_analysis.scores.structure}
                        label={t("tasks:completion.score_structure")}
                      />
                    )}
                    {llm_analysis.scores.relevance !== undefined && (
                      <ScoreBar
                        score={llm_analysis.scores.relevance}
                        label={t("tasks:completion.score_relevance")}
                      />
                    )}
                  </div>
                </div>
            ...
            {/* Improvements */}
            {llm_analysis.improvements &&
              llm_analysis.improvements.length > 0 && (
                <div>
                  <div className="mb-3 flex items-center gap-2">
            ...
            {/* Highlighted Phrases */}
            {llm_analysis.highlighted_phrases &&
              llm_analysis.highlighted_phrases.length > 0 && (
                <div>
                  <div className="mb-3 flex items-center gap-2">
