...
class WritingJSONParser(BaseWorkflowTool):
    ...
    def parse_evaluation(
        self, text: str
    ) -> Tuple[int, List[WritingCriteriaDimension], str, str, str]:
...
class WritingDatabaseManager(BaseWorkflowTool):
    ...
    def save_writing(self, state: WritingWorkflowState) -> str:
        ...
        result = self.mongodb[CollectionName.ENG_WRITINGS.value].insert_one(
            data.model_dump()
        )
    ...
    def get_recent_writings(self, n: int = 10) -> List[Dict]:
        ...
        cursor = (
            self.mongodb[CollectionName.ENG_WRITINGS.value]
            .find()
            .sort("created_at", -1)
            .limit(n)
        )
    ...
    def get_writing_by_id(self, writing_id: str) -> Optional[Dict[str, Any]]:
        ...
        result = self.mongodb[CollectionName.ENG_WRITINGS.value].find_one(
...
class WritingAnalysisTools(BaseWorkflowTool):
    ...
    def get_avg_score_by_type(self, essay_type: str = None) -> float:
        ...
        scores = [
            doc.get("overall_score", 0) for doc in cursor if doc.get("overall_score")
        ]
    ...
    def get_common_weaknesses(self, n: int = 5) -> List[Dict]:
        ...
        return list(
            self.db.mongodb[CollectionName.ENG_WRITINGS.value].aggregate(pipeline)
        )
    ...
    def _generate_improvement_suggestions(self, writing: Dict) -> List[str]:
        ...
        for weakness in weaknesses:
            if "spelling" in weakness.lower():
                suggestions.append(
                    "Practice spelling common words and use a dictionary"
                )
                ...
                suggestions.append(
                    "Practice organizing ideas with an outline before writing"
                )
